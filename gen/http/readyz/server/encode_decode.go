// Code generated by goa v3.16.1, DO NOT EDIT.
//
// readyz HTTP server encoders and decoders
//
// Command:
// $ goa gen k8s-federated-credential-api/design

package server

import (
	"context"
	readyz "k8s-federated-credential-api/gen/readyz"
	"net/http"

	goahttp "goa.design/goa/v3/http"
)

// EncodeReadyzResponse returns an encoder for responses returned by the readyz
// readyz endpoint.
func EncodeReadyzResponse(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder) func(context.Context, http.ResponseWriter, any) error {
	return func(ctx context.Context, w http.ResponseWriter, v any) error {
		res, _ := v.(*readyz.ReadyzResult)
		enc := encoder(ctx, w)
		body := NewReadyzResponseBody(res)
		w.WriteHeader(http.StatusOK)
		return enc.Encode(body)
	}
}
